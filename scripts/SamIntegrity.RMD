...
```{r}
TestBamFileList <- Rsamtools::BamFileList(c("/media/guillaume-chanfreau/HD3/TRAMP_Direct/TRF4_3.bam", "/media/guillaume-chanfreau/HD3/TRAMP_Direct/slu7AA_IVP.bam"))

YeastCDSGFF <- import.gff("/home/guillaume-chanfreau/Annotation_files/saccharomyces_cerevisiae.20210411.CDS.no_mito.gff")

TRF4slu7Integrity <- calculateIntegrity(GeneTargets = YeastCDSGFF, BamFiles = TestBamFileList)
```
```{r}
for (file_name in levels(TRF4slu7Integrity$file)) {
	print(file_name)
	PlotData <- TRF4slu7Integrity[c(TRF4slu7Integrity$file == file_name),]
	
	PlotData$TranscriptNumber <- as.numeric(str_split_i(row.names(PlotData), "\\.", -1))
	
	IntegrityPlot <-ggplot(data = PlotData)+
